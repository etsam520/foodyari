...
<body class=" ">
    ...
    <!-- Select2 Global Protection -->
    <script>
        // Protect against Select2 errors by creating a safe wrapper
        (function($) {
            if (typeof $.fn.select2 === 'undefined') {
                // Create a dummy select2 function to prevent errors
                $.fn.select2 = function(options) {
                    console.warn('Select2 not loaded yet, queuing initialization...');
                    var elements = this;
                    
                    // Queue the initialization for when Select2 is available
                    $(document).on('select2Ready', function() {
                        if (typeof $.fn.select2 !== 'undefined' && $.fn.select2.Constructor) {
            ...
            setTimeout(function() {
                clearInterval(checkSelect2);
                if (typeof $.fn.select2 === 'undefined' || !$.fn.select2.Constructor) {
                    console.error('Select2 failed to load properly');
                }
            }, 5000);
        })(jQuery);
    </script>
    ...
    <script>
        ...
        function initializeSelect2(container = document) {
            try {
                if (typeof $.fn.select2 !== 'undefined' && $.fn.select2.Constructor) {
                    // Initialize select elements with class select-2
                    $(container).find('select.select-2').each(function() {
                        if (!$(this).hasClass('select2-hidden-accessible')) {
                            try {
                                $(this).select2();
                            } catch (e) {
                                console.error('Error initializing select-2:', e);
                            }
                        }
                    });
                    
                    // Initialize select elements with class js-select2-custom
                    $(container).find('.js-select2-custom').each(function() {
                        if (!$(this).hasClass('select2-hidden-accessible')) {
                            try {
                                $(this).select2({
                                    width: '100%',
                                ...
                                console.error('Error initializing js-select2-custom:', e);
                            }
                        }
                    });
                } else {
                    console.warn('Select2 library is not available for initialization');
                }
            } catch (error) {
                console.error('Fatal error in initializeSelect2:', error);
            }
        }
        ...
        $(document).ready(function() {
            try {
                // Initialize on Select2 ready event
                $(document).on('select2Ready', function() {
                    console.log('Select2 ready event received, initializing elements...');
                    initializeSelect2();
                });
                ...
                const tryInitialize = function() {
                    attempts++;
                    if (typeof $.fn.select2 !== 'undefined' && $.fn.select2.Constructor) {
                        console.log('Select2 available, initializing (attempt ' + attempts + ')');
                        initializeSelect2();
                    } else if (attempts < maxAttempts) {
                        setTimeout(tryInitialize, 500);
                    } else {
                        console.warn('Give up waiting for Select2 after ' + maxAttempts + ' attempts');
                    }
                };
                
                setTimeout(tryInitialize, 100);
                
                // Modern mutation observer instead of deprecated DOMNodeInserted
                            ...
                            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                                mutation.addedNodes.forEach(function(node) {
                                    if (node.nodeType === 1) { // Element node
                                        const $node = $(node);
                                        if ($node.find('select').length > 0 || $node.is('select')) {
                                            setTimeout(function() {
        ...
        // Global function to safely reinitialize Select2
        window.reinitializeSelect2 = function(container) {
            try {
                ...
                console.error('Error in reinitializeSelect2:', error);
            }
        };
    </script>
    <script>
    ...
    <script>
        function confirmOrder(orderId,title) {
            ...
            if (result.isConfirmed) {
                location.href = `{{route('admin.order.order-status-update')}}?id=${orderId}&order_status=confirmed`;
                ...
                if (cancelResult.isConfirmed) {
                    ...
                    location.href = `{{ route('admin.order.order-status-update')}}?id=${orderId}&order_status=canceled&cancel_reason=${reason}`;
                }
                });
            }
            });
