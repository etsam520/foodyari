...
@push('javascript')
    <script>
        async function getRestaurants(filter = "all", options = {}) {
            let url = `{{ route('user.restaurant.collection.restaurants') }}`;
            try {
    ...
    <script>
        ...
        async function getFoods(filter = null) {
            let url = "{{ route('user.restaurant.collection.foods') }}";
    ...
    <script>
        ...
        async function undoFromCollection(item) {
            ...
            const result = await resp.json();
            Swal.fire({title: result.message,icon: "success"});
            item.closest('.restaurant-item').remove();
        }
