...
@section('containt')
    ...
    <div class="d-lg-none d-block m-1">
        <div class="bg-primary d-flex w-100 rounded-4">
            <h4 class="text-white fw-bolder fs-4 me-auto mb-0 py-3 px-4" onclick="window.history.back()"
                style="border-right: 1px solid white;">
                <i class="fas fa-arrow-left"></i>
            </h4>
            <h4 class="fw-bold m-0 text-white w-100 align-self-baseline text-center p-3 ps-0"><i
                    class="fas fa-shopping-bag me-2"></i>Pay Options</h4>
        </div>
    </div>
        ...
        <div class="row m-0 justify-content-center">
            <div class="col-md-8 px-0 border-top">
                <div class="p-3 mt-4 bg-light rounded-4 w-100 d-flex justify-content-between">
                    {{-- @dd($billData->billingTotal) --}}
                    <h5 class="fw-bolder mb-0">Amount Payable</h5>
                    <h4 class="fw-bolder mb-0">{{ Helpers::format_currency(ceil($billData->billingTotal)) }}</h4>
                </div>
                    ...
                    <div class="mb-0 p-0 d-flex justify-content-between align-items-center">
                        <div class="w-100 d-flex justify-content-between align-items-center p-3">
                            <div class="align-self-center">
                                <input class="form-check-input mt-0" type="checkbox" name="paymode['wallet']"
                                    {{ $wallet->balance <= 0 ? 'disabled' : null }} value="wallet" id="wallet-check"
                            ...
                            <div class="text-end d-flex">
                                @if ($wallet->balance < $billData->billingTotal)
                                    <div class="text-warning text-nowrap align-self-center me-2">Low Balance :
                                        {{ App\CentralLogics\Helpers::format_currency($wallet->balance) }}</div>
                                @else
                                    <div class="text-success text-nowrap align-self-center me-2">Balance :
                                        {{ App\CentralLogics\Helpers::format_currency($wallet->balance) }}</div>
                                @endif
                                <a class="btn btn-primary text-nowrap text-white" data-bs-toggle="offcanvas"
                                    href="#wallet-top-up" role="button" aria-controls="walletTopUP">Top-up</a>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="bg-white rounded-4 w-100 mt-3" id="online-getway" style="visibility: hidden; position: absolute; z-index: 0;">
                    ...
                    <div class="mb-0 p-3" >
                        <div class="">
                            <input class="form-check-input" name="gateway" type="radio" value="phonepe"
                                id="flexCheckDefault" style="font-size: 20px;border:1px solid #ff810a;">
                            <label class="form-check-label ms-3 fx-6" for="flexCheckDefault">
                                <img src="{{ asset('assets/user/img/phonepe.webp') }}" alt="" style="height: 25px;"
                                    class="me-1">
                                Online
                            </label>
                        </div>
                ...
                <div class="bg-white mt-3 rounded-4 w-100" id="cash-container">
                    <div class="mb-0 p-3">
                        <div class="">
                            <input class="form-check-input" type="radio" name="paymode['cash']" value="cash"
                                id="cash" style="font-size: 20px;border:1px solid #ff810a;">
                ...
                <p class="mt-3"><span class="text-danger me-2">*</span><small>PLEASE KEEP THE CHANGE FOR FASTER
                        DELIVERY</small></p>
            </div>
        </div>
    </div>
        ...
        <div class="offcanvas-body">
            <div class="row">
                <div class="form-group col-md-6 mx-auto">
                    <form action="{{ route('user.wallet.top-up') }}" method="post"
                        class="p-3 d-flex flex-column justify-content-around" method="post">
                        @csrf
                        <label class="form-label mx-2" for="add-amount ">Add Amount:</label>
                        <input type="number" class="form-control " name="amount" value="" id="add-amount"
                            placeholder="0">
                        </span>
                        <div class="bg-white rounded-4 w-100 mt-3">
                            <h6 class="fw-bolder mb-0 p-3 border-bottom"><i
                                    class="fa-solid fa-money-check text-warning me-2"></i>Digital Payment</h6>
                            {{-- <div class="mb-0 p-3">
                        <div class="">
                            ...
                            <div class="mb-0 p-3">
                                <div class="">
                                    <input class="form-check-input" name="gateway" type="radio" value="phonepe"
                                        id="top-up-gatway2" style="font-size: 20px;border:1px solid #ff810a;">
...
@push('javascript')
    ...
    <script type="module">
        // Utility function to handle floating-point precision
        function roundToTwoDecimals(num) {
            return Math.round((num + Number.EPSILON) * 100) / 100;
        }

        const CALC_OPTION = {
            orderAmount: {{ $billData->billingTotal }},
        ...
        walletCheckInput.addEventListener('change', () => {
            try {
                if (CALC_OPTION.walletBalance <= 0 || !walletCheckInput.checked) {
                    ...
                    if (CALC_OPTION.walletBalance <= 0) {
                        throw new Error('Insufficient Wallet Amount');
                    }
                    return;
                }
                ...
                Swal.fire({
                    ...
                    inputValue: CALC_OPTION.wallet > CALC_OPTION.orderAmount ? CALC_OPTION.orderAmount.toFixed(2) : CALC_OPTION.wallet.toFixed(2),
                    showCancelButton: true,
                    confirmButtonText: "OK",
                    preConfirm: (wallet_amount) => {
                        try {
                            wallet_amount = parseFloat(
                                wallet_amount); // Ensure the input is treated as a number
                            ...
                            CALC_OPTION.wallet = roundToTwoDecimals(wallet_amount);
                            CALC_OPTION.pluckAmount = roundToTwoDecimals(CALC_OPTION.orderAmount - CALC_OPTION.wallet);
                            
                            // Handle floating-point precision issues - if pluckAmount is very small, set to 0
                            if (Math.abs(CALC_OPTION.pluckAmount) < 0.01) {
                                CALC_OPTION.pluckAmount = 0;
                            }

                            if (CALC_OPTION.pluckAmount != 0 && CALC_OPTION.wallet > 0) {
                                const message = `Rs. ${CALC_OPTION.wallet.toFixed(2)} will be used from the wallet and the remaining amount of Rs. ${CALC_OPTION.pluckAmount.toFixed(2)} is payable in cash or through the Payment Gateway`;
                                Swal.fire({
                                    title: message,
                    ...
                    console.log("wallate confirmation " ,result , "calc option" , CALC_OPTION);
                    if (result.isConfirmed) {
                        // else if(Math.ceil(wallet_amount) == Math.ceil(CALC_OPTION.orderAmount)){
                        //         throw new Error('Amount must be an integer value');
                        //     }
                        if (Math.ceil(CALC_OPTION.pluckAmount) == 0) {
        ...
        getwayOptions.forEach(element => {
            element.addEventListener('change', () => {
                try {
                    ...
                    if (CALC_OPTION.pluckAmount > 0 && CALC_OPTION.wallet > 0) {
                        CALC_OPTION.online = roundToTwoDecimals(CALC_OPTION.pluckAmount);
                        message =
                            `Rs. ${CALC_OPTION.wallet.toFixed(2)} will be used from the wallet and the remaining amount of Rs. ${CALC_OPTION.pluckAmount.toFixed(2)} is payable via the Payment Gateway.`;
                    } else {
                        CALC_OPTION.online = roundToTwoDecimals(CALC_OPTION.orderAmount);
                        message =
                            `Rs. ${CALC_OPTION.online.toFixed(2)} will be paid using the Payment Gateway.`;
                    }
                    ...
                    Swal.fire({
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                // Format all numeric values to 2 decimal places using utility function
                                const formattedOptions = {
                                    orderAmount: roundToTwoDecimals(CALC_OPTION.orderAmount),
                                    walletBalance: roundToTwoDecimals(CALC_OPTION.walletBalance),
                                    cash: roundToTwoDecimals(CALC_OPTION.cash),
                                    wallet: roundToTwoDecimals(CALC_OPTION.wallet),
                                    online: roundToTwoDecimals(CALC_OPTION.online),
                                    pluckAmount: roundToTwoDecimals(CALC_OPTION.pluckAmount),
                                    gateway: CALC_OPTION.gateway
                                };
                                
                                // Remove zero values except for gateway
                                const cleanOptions = {};
                                Object.entries(formattedOptions).forEach(([key, value]) => {
                                    if (key === 'gateway' || (typeof value === 'number' && value !== 0) || (typeof value === 'string' && value !== '')) {
                                        cleanOptions[key] = value;
                                    }
                                });
                                
                                const URI = new URLSearchParams(cleanOptions).toString();

                                location.href =
                                    "{{ route('user.restaurant.order-payment-online') }}?" +
                                    URI;
        ...
        cashContainer.addEventListener('change', () => {
            try {
                ...
                let message = '';
                if (CALC_OPTION.wallet > 0) {
                    CALC_OPTION.cash = roundToTwoDecimals(CALC_OPTION.pluckAmount);
                    message =
                        `Rs. ${CALC_OPTION.wallet.toFixed(2)} will be used from the wallet and the remaining amount of Rs. ${CALC_OPTION.cash.toFixed(2)} is payable via Cash.`;
                } else {
                    CALC_OPTION.cash = roundToTwoDecimals(CALC_OPTION.orderAmount);
                    message = `Rs. ${CALC_OPTION.cash.toFixed(2)} will be colleted via Cash`;
                }
        ...
        function placeOrder(CALC_OPTION) {
            ...
            // Remove zero values except for gateway which can be null
            const cleanOptions = {};
            Object.entries(formattedOptions).forEach(([key, value]) => {
                if (key === 'gateway' || (typeof value === 'number' && value !== 0) || (typeof value === 'string' && value !== '')) {
                    cleanOptions[key] = value;
            ...
            const URI = new URLSearchParams(cleanOptions).toString();
            location.href = "{{ route('user.restaurant.place-order') }}?" + URI;
        }
    </script>
@endpush
