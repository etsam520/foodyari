            ...
            <div class="initial-38-1">
                ...
                <div class="text-center pt-2 mb-3">
                    ...
                    <h5 class="initial-38-4 initial-38-3">
                        <span>{{ __('phone') }}</span> <span>:</span> <span>{{ $order->restaurant->phone }}</span>
                    </h5>
                    @if ($order->restaurant->gst_status)
                    <h5 class="initial-38-4 initial-38-3 fz-12px">
                        <span>{{ __('Gst No') }}</span> <span>:</span> <span>{{ $order->restaurant->gst_code }}</span>
                    </h5>
                    @endif
                ...
                <table class="table table-bordered mt-1 mb-1">
                    <thead>
                        <tr>
                            <th class="initial-38-6">{{ __('messages.qty') }}</th>
                            <th class="initial-38-7">{{ __('messages.DESC') }}</th>
                            <th class="initial-38-7">{{ __('messages.price') }}</th>
                        </tr>
                    </thead>
                ...
                <div class="mb-2 initial-38-9">
                    <div class="px-3">
                        <dl class="row text-right">
                            <dt class="col-6 text-left">{{ __('Items Price') }}:</dt>
                            <dd class="col-6">{{ \App\CentralLogics\Helpers::format_currency($sub_total) }}</dd>
                            <dt class="col-6 text-left">{{ __('Addon Cost') }}:</dt>
                            <dd class="col-6">
                                {{ \App\CentralLogics\Helpers::format_currency($add_ons_cost) }}
                                <hr>
                            </dd>
                            <dt class="col-6 text-left">{{ __('messages.subtotal') }}
                            ...
                            <dt class="col-6 text-left">{{ __('messages.delivery_man_tips') }}:</dt>
                            <dd class="col-6">
                                @php($dm_tips = $order['dm_tips'])
                                {{ \App\CentralLogics\Helpers::format_currency($dm_tips) }}
                            </dd>
                            <dt class="col-6 text-left">{{ __('messages.delivery_charge') }}:</dt>
                            <dd class="col-6">
                            ...
                            <dt class="col-6 text-left fz-20px">{{ __('messages.total') }}:</dt>
                ...
                <div class="d-flex flex-row justify-content-between border-top pt-3">
                    <span class="text-capitalize d-flex"><span>{{ __('Paid by') }}</span> <span>:</span> <span>{{ __(str_replace('_', ' ', $order['payment_method'])) }}</span></span>

                    {{-- <span>{{__('messages.amount')}}: {{$order->adjusment}}</span>
                    <span>{{__('messages.change')}}: {{$order->adjusment - $order->order_amount}}</span> --}}


                </div>
                <span class="initial-38-7">-------------------------------------------------------------------</span>
                <h5 class="text-center pt-1 justify-content-center">
                    <span class="d-block">"""{{ __('THANK YOU') }}"""</span>
                </h5>
                <span class="initial-38-7">-------------------------------------------------------------------</span>
                <span class="d-block text-center">© {{date('Y')}} {{\App\Models\BusinessSetting::where(['key'=>'business_name'])->first()->value}}. {{__('messages.all_right_reserved')}}</span>
            </div>
        </div>
    </div>
</div>
