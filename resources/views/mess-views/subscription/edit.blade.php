...
<div class="conatiner-fluid content-inner mt-n5 py-0">
    <div>
        <form action="{{route('mess.subscription.update')}}"  method="POST" id="cusomer-form" enctype="multipart/form-data">
            @csrf
            ...
            <div class="col-xl-12 col-lg-12">
                <div class="card">
                    ...
                    <div class="header-title">
                        <h4 class="card-title">Update Subscription Pakage</h4>
                    </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" value="{{$subscription->title}}" name="title" id="title" placeholder="Title" required>
                                </div>
                                <div class="form-group">
                                    <label class="form-label">Type</label>
                                    <select name="item_type"  class="selectpicker form-control" data-style="py-0">
                                        <option disabled selected value="">Select One</option>
                                        <option value="veg" {{$subscription->type =='veg'?'selected':null}}>Veg</option>
                                        <option value="non_veg" {{$subscription->type =='non_veg'?'selected':null}} >Non Veg</option>
                                        <option value="both" {{$subscription->type =='both'?'selected':null}}>Both</option>
                                     </select>
                                </div>
                                <div class="form-group">
                                    <label class="form-label" for="sdate">Validity:</label>
                                    <input type="text"  name="validity" id="sdate" required class="form-control "
                                    value="{{$subscription->validity}}" placeholder="No. of days">  
                                </div>
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="form-group ">
                                    ...
                                    <textarea name="description" class="form-control"id="add1"  cols="10" rows="5">{{$subscription->description}}</textarea>
                                </div>
                            </div>
                            <div class="form-group col-md-6"> 
                                <label class="form-label" for="price">Price:</label>
                                <input type="text" class="form-control" required name="price" value="{{$subscription->price}}"  id="price" placeholder="Ex. 20">
                            </div>
                            <div class="col-md-6">
                                ...
                                <div class="form-group d-flex " id="speciality">
                                    <div class="form-check d-block"> 
                                        <input class="form-check-input" type="radio" required name="speciality"
                                            value="1" id="special-diet" {{$subscription->speciality == 1? 'checked' : null}}>
                                    ...
                                    <div class="form-check d-block mx-3">
                                        <input class="form-check-input" type="radio"  required name="speciality"
                                            value="0" id="normal-diet"  {{$subscription->speciality == 0? 'checked' : null}}>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="discount_type" class="form-control js-select2-custom">
                                        ...
                                        <option value="percent"  {{$subscription->discount_type == 'percent'? 'selected' : null}}>Percent (%)</option>
                                        <option value="amount" {{$subscription->discount_type == 'amount'? 'selected' : null}}>Amount ($)</option>
                                    </select> 
                                </div>
                                <div class="form-group ">
                                    <label class="form-label"
                                    ...
                                    <input type="number" min="0" max="9999999999999999999999"
                                        name="discount" class="form-control" value="{{$subscription->discount}}"
                            ...
                            <div class=" col-md-6">
                                <div class="form-group d-none" id="Special-Diet">
                                    ...
                                    <input type="text" class="form-control" data-diet="special" name="no_diet_special" id="pno" 
                                    value="{{$subs['special']}}" placeholder="Ex. 30">
                                </div>
                                <label class="form-label"  >No. of Diets(<i data-diet-count="{{$dietCount}}">{{$dietCount}}</i>):</label>
                                <div class="d-flex"  >
                                    <div class="form-group">
                                        <label class="form-label" for="breakfast">Breakfast:</label>
                                        <input type="text" class="form-control" data-diet="breakfast" name="no_d_breakfast" id="breakfast"
                                        value="{{$subs['breakfast']}}" required placeholder="Ex. 30">
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label" for="lunch">Luch:</label>
                                        <input type="text" class="form-control" data-diet="lunch" name="no_d_lunch" id="lunch"
                                        value="{{$subs['lunch']}}" required placeholder="Ex. 30">
                                    </div>
                                    <div class="form-group">
                                        <label class="form-label" for="dinner">Dinner:</label>
                                        <input type="text" class="form-control" data-diet="dinner" name="no_d_dinner" id="dinner"
                                        value="{{$subs['dinner']}}" required placeholder="Ex. 30">
                                    </div>
                                </div>
                            </div>
...
const preloaded = [
    @php($images = json_decode($subscription->images))
    @foreach ($images as $image)
        {id : '{{$image}}' , src : "{{asset('messSubscriptionToCustomers/'.$image)}}"  },
    @endforeach
];
@endif
