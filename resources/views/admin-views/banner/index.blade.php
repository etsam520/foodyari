                ...
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title">
                       <h4 class="card-title">
                        ...
                        {{__('messages.add')}} {{__('messages.new')}} {{__('messages.banner')}}
                        </h4>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title" class="form-control" placeholder="{{__('messages.Banner Name')}}" required>
                                </div>
                                    ...
                                    <div class="form-group mt-auto">
                                        <center>
                                            <label class="custom-file-label" for="customFileEg1">
                                                ...
                                                src="{{asset('assets/images/icons/banner.jpg')}}" width="200px" alt="campaign image"/>
                                            </label>
                                        </center>
                                    </div>
                                    <div class="form-group mt-auto">
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="media d-flex align-items-center">
                                        <div class="div"  style="width: 150px; height:100px;">
                                            <img class="avatar avatar-lg mr-3 avatar--3-1" src="{{asset('/banner')}}/{{$banner['image']}}" alt="{{$banner->name}} image" style="width: 100%">
                                        </div>
                                        <div class="media-body mx-2">
                                            <h5 class="text-hover-primary mb-0">{{Str::upper(Str::limit($banner['title'], 25, '...'))}}</h5>
                                        </div>
                                    </span>
                                ...
                                <td>
                                    <label class="form-check toggle-switch-sm" for="statusCheckbox{{$banner->id}}">
                                        <input type="checkbox" onclick="location.href='{{route('admin.banner.status',[$banner['id'],$banner->status?0:1])}}'" class="form-check-input" id="statusCheckbox{{$banner->id}}" {{$banner->status?'checked':''}}>
                                ...
                                <td>
                                    <div class="btn-container justify-content-center">
                                        <a class="btn" href="{{route('admin.banner.edit',['banner_id'=>$banner['id']])}}"title="{{__('messages.edit')}} {{__('messages.banner')}}">
                                           <i class="fa fa-edit text-primary"></i>
                                        </a>
                                        <a class="btn"  href="{{route('admin.banner.delete',$banner['id'])}}" onclick="form_alert(this,'{{__('Want to delete this marquee')}}')" title="{{__('messages.delete')}} {{__('messages.marquee')}}">
...
<div class="modal fade" id="edit-banner-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
...
<script>
    ...
    document.querySelector('#banner_type').addEventListener('change', async event => {
        try {
...
$('[data-zone]').select2({}).on('change', async (event) => {
    try {
        const resp = await fetch(`{{ route('admin.food.getFoodZoneWise') }}?zone_id=${event.target.value}`);
        const result = await resp.json();
        let dataToAppend = [
            {
                id: '',
