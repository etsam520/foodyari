                        ...
                        <div class="d-flex align-items-center flex-wrap">
                            ...
                            <div class="dropdown me-3">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton22" style="">
                                   <li><a class="dropdown-item" href="{!! route('admin.report.tax').'?filter=this_week' !!}">This Week</a></li>
                                   <li><a class="dropdown-item" href="{!! route('admin.report.tax').'?filter=this_month' !!}">This Month</a></li>
                                   <li><a class="dropdown-item" href="{!! route('admin.report.tax').'?filter=this_year' !!}">This Year</a></li>
                                   <li><a class="dropdown-item" href="{!! route('admin.report.tax').'?filter=previous_year' !!}">Previous Year</a></li>
                                   <li><a class="dropdown-item" href="{!! route('admin.report.tax').'?filter=today' !!}">Today</a></li>
                                </ul>
                            </div>
                            <form action="{{route('admin.report.tax')}}">
                        ...
                        <table class="table table-responsive dataTable" id="datatable-export" data-toggle="data-table-export" aria-describedby="datatable_info">
                            <thead>
                                <tr>
                                    ...
                                    <th>(C)Gross Total</th>
                                    <th>(C)GST (%)</th>
                                    <th>(C)GST ({{Helpers::currency_symbol()}}) </th>

                                    <th>(A)Gross Total</th>
                                    <th>(A)GST ({{Helpers::currency_symbol()}})</th>
                                    <th>(A)Total</th>
                                    <th>(A)Earning</th>
                                     <!-- Here Order Price Should be restaurant Order amount not customer order amount -->
                                    <th>(R)Gross Total</th>
                                    <th>(R)GST (%)</th>
                                    <th>(R)GST ({{Helpers::currency_symbol()}})</th>
                                    <th>(R)Total</th>
                                    <th>(R)Earning</th>
                                </tr>
                            </thead>
                            @php
                                $c_sumGrossTotal= $c_sumGstAmount = $a_sumGrossMargin = $a_sumGstAmount = $a_sumReceivableAmount=
                                $a_sumEarning =$r_sumGrossTotal=$r_sumGstAmount=$r_sumReceivableAmount = $r_sumEarning = 0;
                            @endphp
                            <tbody>
                                @foreach ($transactions as $key => $txn)
                                @php($restaurantOrderData = json_decode($txn->restaurant_data))
                                @php($adminOrderData = json_decode($txn->admin_data))
                                @php($customerOrderData = json_decode($txn->customer_data))
                                {{-- @dd($adminOrderData) --}}
                                    <tr>
                                        <td>{{$key + 1}}</td>
                                        ...
                                        <td>{{Helpers::format_currency($customerOrderData?->grossTotal)}}</td>
                                        @php($c_sumGrossTotal += (float) $customerOrderData?->grossTotal)

                                        <td>{{$customerOrderData?->gstPercent}}</td>

                                        <td>{{Helpers::format_currency($customerOrderData?->gstAmount)}}</td>
                                        @php($c_sumGstAmount += (float) $customerOrderData?->gstAmount)



                                        <td>{{Helpers::format_currency($adminOrderData?->grossMargin)}}</td>
                                        @php($a_sumGrossMargin += (float) $adminOrderData?->grossMargin)

                                        <td>{{Helpers::format_currency($adminOrderData?->gstAmount)}}</td>
                                        @php($a_sumGstAmount += (float) $adminOrderData?->gstAmount)

                                        <td>{{Helpers::format_currency($adminOrderData?->receivableAmount)}}</td>
                                        @php($a_sumReceivableAmount += (float) $adminOrderData?->receivableAmount)

                                        <td>{{Helpers::format_currency($adminOrderData?->earning)}}</td>
                                        @php($a_sumEarning += (float) $adminOrderData?->earning)


                                        <td>{{Helpers::format_currency($restaurantOrderData?->grossTotal)}}</td>
                                        @php($r_sumGrossTotal += (float) $restaurantOrderData?->grossTotal)

                                        <td>{{$restaurantOrderData?->gstPercent." %"}}</td>
                                        <td>{{Helpers::format_currency($restaurantOrderData?->gstAmount)}}</td>
                                        @php($r_sumGstAmount += (float) $restaurantOrderData?->gstAmount)

                                        <td>{{Helpers::format_currency($restaurantOrderData?->receivableAmount)}}</td>
                                        @php($r_sumReceivableAmount += (float) $restaurantOrderData?->receivableAmount)

                                        <td>{{Helpers::format_currency($restaurantOrderData?->earning)}}</td>
                                        @php($r_sumEarning += (float) $restaurantOrderData?->earning)
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                <tr>
                                    ...
                                    <th><small> (C)Gross Total
                                        <br>{{Helpers::format_currency($c_sumGrossTotal)}} </small></th>
                                    <th><small>(C)GST (%)</small></th>
                                    <th><small>(C)GST ({{Helpers::currency_symbol()}})
                                        <br>{{Helpers::format_currency($c_sumGstAmount)}}</small></th>


                                    <th><small> (A)Gross Total
                                        <br>{{Helpers::format_currency($a_sumGrossMargin)}} </small></th>

                                    <th><small> (A)GST ({{Helpers::currency_symbol()}})
                                        <br>{{Helpers::format_currency($a_sumGstAmount)}} </small></th>

                                    <th><small> (A)Total
                                        <br>{{Helpers::format_currency($a_sumReceivableAmount)}} </small></th>

                                    <th><small> (A)Earning
                                        <br>{{Helpers::format_currency($a_sumEarning)}} </small></th>


                                    <th><small> (R)Gross Total
                                        <br>{{Helpers::format_currency($r_sumGrossTotal)}} </small></th>

                                    <th><small>(R)GST (%)</small></th>


                                    <th><small> (R)GST ({{Helpers::currency_symbol()}})
                                        <br>{{Helpers::format_currency($r_sumGstAmount)}}</small></th>

                                    <th><small> (R)Total
                                        <br>{{Helpers::format_currency($r_sumReceivableAmount)}}</small></th>

                                    <th><small> (R)Earning
                                        <br>{{Helpers::format_currency($r_sumEarning)}}</small></th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
