                        ...
                        <div class="d-flex align-items-center flex-wrap">
                            ...
                            <div class="dropdown me-3">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton22" style="">
                                   <li><a class="dropdown-item" href="{!! route('admin.report.order').'?filter=this_week' !!}">This Week</a></li>
                                   <li><a class="dropdown-item" href="{!! route('admin.report.order').'?filter=this_month' !!}">This Month</a></li>
                                   <li><a class="dropdown-item" href="{!! route('admin.report.order').'?filter=this_year' !!}">This Year</a></li>
                                   <li><a class="dropdown-item" href="{!! route('admin.report.order').'?filter=previous_year' !!}">Previous Year</a></li>
                                   <li><a class="dropdown-item" href="{!! route('admin.report.order').'?filter=today' !!}">Today</a></li>
                                </ul>
                            </div>
                            <form action="">
                        ...
                        <table class="table table-responsive dataTable" id="datatable-export" data-toggle="data-table-export" aria-describedby="datatable_info">
                            <thead>
                                <tr>
                                    ...
                                    <th>-Discount</th>
                                    <th>Coupon Code</th>
                                    <th>-Coupon Dis.({{Helpers::currency_symbol()}}) </th>
                            ...
                            @php
                                $sumFoodsAmount = $sumDiscountAmount = $sumCouponDiscountAmount = $sumPlatformCharge = $sumofPackingCharge = $sumofDliveryChargee = $sumGrossTotal = $sumGstAmount = $sumDmTips = $sumBillingTotal = 0;
                            @endphp
                            <tbody>
                                @foreach ($transactions as $txn)
                                {{-- @dd($txn) --}}
                                @php($customerData = json_decode($txn->customer_data))
                                {{-- @dd($customerData) --}}
                                    <tr>
                                        <td><a href="{{route('admin.order.details', $txn->order_id)}}">#{{$txn->order_id}}</a></td>
                                        <td>
                                        ...
                                        <td>{{Str::ucfirst($customerData?->restaurant?->name)}}</td>
                                        <td>{{Str::ucfirst($customerData?->zone?->name)}}</td>

                                        <td>{{Helpers::format_currency($customerData?->sumOfFoodPriceBeforDiscount)}}</td>
                                        @php($sumFoodsAmount += (float) ($customerData?->sumOfFoodPriceBeforDiscount))

                                        <td>{{Helpers::format_currency($customerData?->sumOfDiscount)}}</td>
                                        @php($sumDiscountAmount += (float) ($customerData?->sumOfDiscount))

                                        {{-- <td>{{$customerData?->couponDiscount." %"}}</td> --}}
                                       <td>{{ $customerData?->couponDetails[0]->code ?? '' }}</td>

                                        <td>{{Helpers::format_currency($customerData?->couponDiscountAmount)}}</td>
                                        @php($sumCouponDiscountAmount += (float) ($customerData?->couponDiscountAmount))

                                        <td>{{Helpers::format_currency($customerData?->sumofPackingCharge)}}</td>
                                        @php($sumPlatformCharge += (float) ($customerData?->sumofPackingCharge))

                                        <td>{{Helpers::format_currency($customerData?->sumofPackingCharge)}}</td>
                                        @php($sumofPackingCharge += (float) ($customerData?->sumofPackingCharge))

                                        <td>{{Helpers::format_currency($customerData?->deliveryCharge)}}</td>
                                        @php($sumofDliveryChargee += (float) ($customerData?->deliveryCharge))

                                        <td>{{Helpers::format_currency($customerData?->grossTotal)}}</td>
                                        @php($sumGrossTotal += (float) ($customerData?->grossTotal))

                                        <td>{{$customerData?->gstPercent." %"}}</td>
                                        <td>{{Helpers::format_currency($customerData?->gstAmount)}}</td>
                                        @php($sumGstAmount += (float) ($customerData?->gstAmount))

                                        <td>{{Helpers::format_currency($customerData?->dm_tips)}}</td>
                                        @php($sumDmTips += (float) ($customerData?->dm_tips))

                                        <td>{{Helpers::format_currency($customerData?->billingTotal)}}</td>
                                        @php($sumBillingTotal += (float) ($customerData?->billingTotal))

                                        {{-- <td>{{Helpers::format_currency($customerData?->earning)}}</td>
                                        @php($sumEarning += (float) ($customerData?->earning)) --}}
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                <tr>
                                    ...
                                    <th></th>
                                    <th></th>
                                    <th><small> Food Price  <!-- Here Order Price Should be restaurant Order amount not customer order amount -->
                                        <br>{{Helpers::format_currency($sumFoodsAmount)}}</small></th>
                                    <th><small> -Discount
                                        <br>{{Helpers::format_currency($sumDiscountAmount)}}</small></th>
                                    <th></th>
                                    <th><small> -Coupon Dis.({{Helpers::currency_symbol()}})
                                        <br>{{Helpers::format_currency($sumCouponDiscountAmount)}}</small></th>
                                    <th><small> Platform Charge
                                        <br>{{Helpers::format_currency($sumPlatformCharge)}}</small></th>
                                    <th><small> Packing Charge
                                        <br>{{Helpers::format_currency($sumofPackingCharge)}}</small></th>
                                    <th><small> Delivery Charge
                                        <br>{{Helpers::format_currency($sumofDliveryChargee)}}</small></th>
                                    ...
                                    <th><small> DM Tips
                                        <br>{{Helpers::format_currency($sumDmTips)}}</small></th>
                                    <th><small> Billing Total
                                        <br>{{Helpers::format_currency($sumBillingTotal)}}</small></th>
                                </tr>
                            </tfoot>
                        </table>
                        {{-- {{ $transactions->links() }} --}}
