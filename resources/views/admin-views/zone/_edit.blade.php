                                ...
                                <div class="col-md-12 col-xl-12 zone-setup">
                                    <div class="pl-xl-5 pl-xxl-0">
                                        <div class="form-group mb-3">
                                            <label class="input-label" for="exampleFormControlInput1">Zone Name</label>
                                            <input id="name" type="text" name="name" class="form-control h--45px"
                                                placeholder="Enter Zone Name" value="{{ $zone->name ?? old('name') }}"
                                                required>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12">
                                            ...
                                            <div class="col-sm-4">
                                                <div class="form-group mb-3">
                                                    <label
                                                        class="input-label text-capitalize d-inline-flex alig-items-center"
                                                        for="max_cod_order_amount">Maximum COD order amount (₹)</label>

                                                    <input type="number" name="max_cod_order_amount" class="form-control"
                                                        id="max_cod_order_amount" min="0"
                                                        value="{{ $zone->max_cod_order_amount ?? old('max_cod_order_amount') }}"
                                                        placeholder="Enter Maximum COD Order Amount">
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="form-group mb-0">
                                                    <label class="input-label" for="exampleFormControlInput1">Platform
                                                        Charge</label>
                                                    <input type="number" min="0" max="999999999999.99"
                                                        value="{{ $zone->platform_charge ?? old('platform_charge') }}"
                                                        name="platform_charge" class="form-control"
                                                        placeholder="Enter Price" required="">
                                                </div>
                                            </div>
                                            <div class="col-md-4">
                                                <div class="form-group mb-0">
                                                    <label class="input-label" for="exampleFormControlInput12">Platform
                                                        Charge Original</label>
                                                    <input type="number" min="0" max="999999999999.99"
                                                        value="{{ $zone->platform_charge_original ?? old('platform_charge_original') }}"
                                                        name="platform_charge_original" class="form-control"
                                                        placeholder="Enter Price" required="">
                                                </div>
                                            </div>
                                            <div class="col-lg-3 col-sm-6">
                                                <div class="form-group">
                                                    <label class="input-label"
                                                        for="exampleFormControlInput1">{{ __('messages.min') }}
                                                        {{ __('messages.purchase') }}</label>
                                                    <input id="min_purchase" type="number" step="0.01"
                                                        name="min_purchase"
                                                        value="{{ $zone->min_purchase ?? old('min_purchase') }}"
                                            ...
                                            <div class="col-lg-4 col-sm-6 mt-2">
                                                <div class="form-group m-0">
                                                    <label
                                                        ...
                                                        for="isTopOrders">
                                                        <span class="pr-2 text-capitalize"> Show Top Orders: <span
                                                                class="input-label-secondary">
                                                            </span>
                                                        </span>
                                                        <input type="checkbox" class="form-check-input"
                                                            onchange="this.value = this.checked?1:0" name="isTopOrders"
                                                            {{ $zone?->isTopOrders ? 'checked' : null }} id="isTopOrders">
                                            ...
                                            <div class="col-lg-4 col-sm-6 mt-2">
                                                <div class="form-group m-0">
                                                    <label
                                                        class="form-check  d-flex justify-content-between border rounded px-3 form-control"
                                                        for="delivery_verification">
                                                        <span class="pr-2 text-capitalize"> Delivery Varification: <span
                                                                class="input-label-secondary">
                                                            </span>
                                                        </span>
                                                        <input type="checkbox" class="form-check-input"
                                                            name="delivery_verification"
                                                            {{ $zone?->delivery_verification ? 'checked' : null }}
                                                            id="delivery_verification">

                                                    </label>
                                                </div>
                                            </div>
                                            @php($coordinates = json_decode($zone->coordinates))
                                            <div class="col-sm-6">
                                                <div class="form-group">
                                                    <label class="input-label" for="latitude">Latitude</label>
                                                    <input type="text" id="latitude" name="latitude"
                                                        class="form-control h--45px disabled" placeholder="Ex : -94.22213"
                                                        value="{{ $coordinates->latitude ?? old('latitude') }}"
                                            ...
                                            <div class="col-sm-6">
                                                <div class="form-group mb-md-0">
                                                    ...
                                                    <input type="text" name="longitude"
                                                        ...
                                                        value="{{ $coordinates->longitude ?? old('longitude') }}"
                                                        placeholder="Ex : 103.344322" id="longitude" value=""
                                                        required="" readonly="">
                                                </div>
                                            </div>
...
@push('javascript')
    ...
    <script>
        ...
        function loadExistingPolygon() {
            // Try to load existing polygon from Laravel zone coordinates
            @if ($zone->coordinates)
                try {
                    const zoneCoordinatesStr = '{!! addslashes($zone->coordinates) !!}';
                    console.log('Loading zone coordinates:', zoneCoordinatesStr);
                    
                    let coordinates;
                    
                    // Check if coordinates are already in array format or need parsing
                    if (typeof zoneCoordinatesStr === 'string') {
                        coordinates = JSON.parse(zoneCoordinatesStr);
                    } else {
                        coordinates = zoneCoordinatesStr;
                    }
                    
                    // Handle different coordinate formats
                    let polygonPath = [];
                    
                    // Check if it's the new format with polygon data
                    if (coordinates.type === 'polygon' && coordinates.polygon && Array.isArray(coordinates.polygon)) {
                        polygonPath = coordinates.polygon.map(coord => ({
                        ...
                        // If it's already an array of coordinate objects (old format)
                        polygonPath = coordinates.map(coord => ({
                            lat: parseFloat(coord.lat || coord.latitude || coord[0]),
                            lng: parseFloat(coord.lng || coord.longitude || coord[1])
                        }));
                    } else if (coordinates.latitude && coordinates.longitude) {
                        // If it's a single center point, create a small square around it
                        const centerLat = parseFloat(coordinates.latitude);
                        const centerLng = parseFloat(coordinates.longitude);
                        const radius = 0.005; // ~500m radius for a smaller default polygon
                    ...
                    if (polygonPath.length >= 3) {
                        ...
                        document.getElementById('zone-coordinates').value = JSON.stringify(polygonPath);
                        
                        // Add polygon listeners
                        ...
                        console.log('Polygon loaded successfully with', polygonPath.length, 'points');
                    } else {
                        console.log('No valid polygon coordinates found, showing center point only');
                    }
                } catch (error) {
                    console.error('Error loading existing polygon:', error);
                    console.log('Raw coordinates data:', '{!! addslashes($zone->coordinates) !!}');
                }
            @endif
        }
            ...
            document.getElementById('reset_btn').addEventListener('click', function() {
                if (confirm('Are you sure you want to reset all changes?')) {
                    ...
                    @if ($coordinates)
                        ...
                        marker.setPosition(originalLocation);
                        setElementsPosition(originalLocation);
                    @endif
