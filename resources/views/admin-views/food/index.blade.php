                    ...
                    <div class="card">
                        ...
                        @endif
                          ...
                          <form class="row g-3 needs-validation" method="POST" enctype="multipart/form-data"  action="{{route('admin.food.store')}}">
                                    ...
                                    <div class="card-body">
                                        <div class="row g-2">
                                            ...
                                            <div class="col-sm-6 col-lg-4">
                                                <div class="form-group mb-0">
                                                    <label class="input-label"
                                                        fo.r="restaurant_menu_id">Menu<span
                                                        class="input-label-secondary text-danger">*</span></label>

                                                    <select name="restaurant_menu_id" id="restaurant_menu_id"  onchange="get_options('{{route('admin.food.get-submenu-option').'?menu_id='}}'+this.value , '#sub-menu-input')"
                                                        class="form-control ">
                                        ...
                                        <div class="row g-2">
                                            <div class="col-md-4" data-food-mode="fixed">
                                                <div class="form-group mb-0">
                                                    ...
                                                    <input type="number" name="restaurant_price" value="{{old('restaurant_price')??0}}" class="form-control"
                                            ...
                                            <div class="col-md-4" data-food-mode="fixed">
                                                <div class="form-group mb-0">
                                                    ...
                                                    <input type="number" min="0" max="999999999999.99"
                                                        step="0.01" id="admin_margin" name="admin_margin" value="{{old('admin_margin')??0}}" class="form-control"
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group mb-0">
                                                    ...
                                                    <input type="number" min="0" max="999999999999.99"
                                                        step="0.01" id="packing_charge" name="packing_charge" value="{{old('packing_charge')??0}}" class="form-control"
                            ...
                            <div class="col-lg-12 d-none" data-food-mode="customize">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title">
                                            <span class="card-header-icon">
                                ...
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                                ...
                                                <div class="col-sm-6">
                                                    <div class="form-group mb-0">
                                                        ...
                                                        <input type="time" name="available_time_ends" class="form-control"
                                                            id="available_time_ends" value="{{old('available_time_ends')}}"
                                                            placeholder="5:45 pm">
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div id="availability-times-container">
                                            <div class="availability-time-row row g-2 align-items-end mb-2" style="display: none;">
                                                <div class="col-md-3">
                                                    <label class="form-label">Day</label>
                                                    <select name="availability_times[0][day]" class="form-control">
                                                ...
                                                <div class="col-md-3">
                                                    ...
                                                    <input type="time" name="availability_times[0][start_time]" class="form-control">
                                                </div>
                                                <div class="col-md-3">
                                                    <label class="form-label">End Time</label>
                                                    <input type="time" name="availability_times[0][end_time]" class="form-control">
                                                </div>
                                                <div class="col-md-3">
                                        ...
                                        <button type="button" id="add-availability-time" class="btn btn-sm btn-outline-primary">
                                            <i class="fas fa-plus"></i> Add Availability Time
                                        </button>
                                    </div>
                                </div>
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', async () => {
            try {
                const resp = await fetch('{{ route("admin.category.get_categories") }}');
...
<script>
    ...
    document.querySelector('#restaurant_id').addEventListener('change', event => {
        get_options("{{ route('admin.addon.get_addons')}}?restaurant_id="+event.target.value,'#add_on');
        get_options("{{ route('admin.restaurant.get_menus')}}?restaurant_id="+event.target.value,'#restaurant_menu_id');
        get_options('{{ url('admin/category/get_categories')}}/'+event.target.value,'#category_id');
...
<script>
// Availability Times Management
document.addEventListener('DOMContentLoaded', function() {
    let availabilityTimeIndex = 0;
    
    // Add new availability time row
    document.getElementById('add-availability-time').addEventListener('click', function() {
        const container = document.getElementById('availability-times-container');
        const template = container.querySelector('.availability-time-row');
        const newRow = template.cloneNode(true);
        
        // Show the row
        newRow.style.display = 'block';
        
        // Update names and IDs to use the current index
        const inputs = newRow.querySelectorAll('input, select');
        inputs.forEach(input => {
            const name = input.name;
            if (name) {
                input.name = name.replace('[0]', '[' + availabilityTimeIndex + ']');
            }
        });
        
        // Add event listener to remove button
        const removeBtn = newRow.querySelector('.remove-availability-time');
        ...
        availabilityTimeIndex++;
    });
    
    // Remove availability time row
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('remove-availability-time')) {
            e.target.closest('.availability-time-row').remove();
        }
    });
});
</script>
