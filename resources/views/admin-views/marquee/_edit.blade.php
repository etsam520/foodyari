    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('admin.marquee.update')}}" method="post" id="marquee_form" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title" value="{{$marquee->title}}" class="form-control" placeholder="{{__('messages.marquee_name')}}" required>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="zone_id" id="zone_select" class="form-control">
                                        ...
                                        <option value="{{$zone->id}}" {{$zone->id == $marquee->zone_id ? 'selected': null}} >{{Str::ucfirst($zone->name)}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="marquee_type" id="marquee_type" class="form-control " data-banner-type="true">
                                        ...
                                        <option value="zone" {{$marquee->type == "zone"  ? 'selected': null}}>{{__('Zone wise')}}</option>
                                        <option value="restaurant" {{$marquee->type == "restaurant"  ? 'selected': null}}>{{__('Restaurant wise')}}</option>
                                        <option value="food" {{$marquee->type == "food"  ? 'selected': null}}>{{__('Food Wise')}}</option>
                                        <option value="location" {{$marquee->type == "location"  ? 'selected': null}}>{{__('Location Wise')}}</option>
                                    </select>
                                </div>
                                    ...
                                    <div class="form-group mt-auto">
                                        <center>
                                            <label class="custom-file-label" for="customFileEg1">
                                                ...
                                                src="{{Helpers::getUploadFile($marquee->file, 'marquee')}}" width="200px" alt="campaign image"></img>
                                            </label>
                                        </center>
                                    </div>
                                    <div class="form-group mt-auto">
...
<script>
    let zone = {{$marquee->zone_id}};
    const typeName = "marquee";
    const typeID = {{$marquee->id}}

    document.querySelector('#zone_select').addEventListener('change', (event) => {
        zone = event.target.value;
    });

    document.querySelector('#marquee_type').addEventListener('change',  (event) => {
...
document.querySelectorAll('[data-edit-button]').forEach(item => {
    item.addEventListener('click', async () => {
        try {
            const marqueeId = item.dataset.editButton;
            console.log(`Fetching data for marquee ID: ${marqueeId}`);
            const resp = await fetch(`{{ route('admin.marquee.edit') }}?marquee_id=${marqueeId}`);

            if (!resp.ok) {
                const errorData = await resp.json();
                // console.error('Error response:', errorData);
                throw new Error(errorData.message || 'Error fetching marquee data');
            }

            const result = await resp.json();
            const targetModal =document.querySelector('#edit-marquee-modal');
            $('#edit-marquee-modal').modal('show');
            targetModal.querySelector('.modal-body').innerHTML = result.view;

            //submit form initiailisation
            submitEditForm('#marquee-edit-form');
