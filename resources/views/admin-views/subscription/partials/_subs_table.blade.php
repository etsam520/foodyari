        ...
        <td>
            @if ($restaurant->vendor->status == 1)
                @if ($restaurant->restaurant_sub_update_application->status == 1)
                    <span class="badge badge-soft-success">{{ __('messages.active') }}</span>
                @else
                <span class="badge badge-soft-danger">{{ __('messages.Expired') }}</span>
                @endif
            @else
                <span class="badge badge-soft-danger">{{ __('messages.inactive_vendor') }}</span>
        ...
        <td>
            <div class="btn--container justify-content-center">
                <a class="btn btn-sm btn--warning btn-outline-warning action-btn"
                    ...
                    title="{{ __('messages.view') }} {{ __('messages.restaurant') }}"><i
