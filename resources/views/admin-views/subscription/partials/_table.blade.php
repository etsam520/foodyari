    ...
    <td>
        <div class="d-flex justify-content-center">
            <label class="toggle-switch toggle-switch-sm" for="stocksCheckbox{{$package->id}}">
                <input type="checkbox"
                 {{-- onclick="status_change_alert('{{route('admin.subscription.package_status',[$package->id,$package->status?0:1])}}',
                '{{$package->status?__('Do You Want To Disable This Package'):__('Do you want to Active This Package')}}', event)" --}}
        ...
        <div class="btn-container justify-content-center">
            <a class="btn btn-sm btn-outline-primary action-btn"
             {{-- href="{{ route('admin.subscription.package_edit',$package->id) }}" --}}
              title="{{__('messages.edit')}} {{__('messages.Package')}}"><i class="fa fa-edit"></i>
            </a>
            <a class="btn btn-sm  btn-outline-warning action-btn"
                href="{{route('admin.subscription.package_details',[$package['id']])}}" title="{{__('messages.view')}} {{__('messages.Package')}}"><i class="fa fa-eye"></i>
            </a>
        </div>
    </td>
