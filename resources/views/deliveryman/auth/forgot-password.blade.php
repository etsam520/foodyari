...
<body>
    <div class="auth-wrapper">
        <div class="module form-module">
            <div id="form-container">
                ...
                <form method="POST" action="javascript:void(0)" id="forgot_password">
                    @csrf
    ...
    <script>
        document.querySelector('#forgot_password').addEventListener('submit', async (event) => {
            event.preventDefault();
            try {

                // toastr.info('Please Wait A While');
                const url = "{{ route('deliveryman.forgot-password-otp') }}";
                ...
                passwordResetForm(document.querySelector('#reset_password'));
        ...
        function passwordResetForm(form){
           form.addEventListener('submit', async (event) => {
                ...
                try {
                    ...
                    // toastr.info('Please Wait A While');
                    const url = "{{ route('deliveryman.forgot-password-save') }}";
                    const form = event.target;
                    const formData = new FormData(form);
                    if (!formData) {
                        throw new Error('Please fill the form properly');
