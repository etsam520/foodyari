...
@include('deliveryman.restaurant.layouts.m-header')      
...
async function loadOrders(params) {
    try {
        const resp = await fetch('{{route('deliveryman.restaurant.get_latest_orders')}}');
        ...
        element.addEventListener('click', ()=> {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const resp = await fetch('{{route('deliveryman.restaurant.order-confirmation')}}?order_id=' + element.dataset.orderId+"&status="+element.dataset.acceptance);
                        if (!resp.ok){
