            ...
            <div class="row justify-content-center pt-3">
                <div class="col-12 col-lg-8">
                    <div class="row">
                        ...
                        <div class="col-6  mb-3">
                            <div class="bg-white shadow-sm rounded p-4 text-center">
                                @php($orderscount = App\Models\Order::where('delivery_man_id', auth('delivery_men')->user()->id)->count())
                                <p class="mb-3 text-primary order-count text-center">{{$orderscount}}</p>
                                <p class="mb-2 order-text">Total Order</p>
                            </div>
                        </div>
                        <div class="col-md-6 col-12 mb-3">
                            <div class="bg-white shadow-sm rounded p-4 text-center" onclick="location.href= '{{route('deliveryman.wallet.index')}}'">
                                <p class="mb-3 text-primary order-count text-center">{{App\CentralLogics\Helpers::format_currency($wallet->balance)}}</p>
                                <p class="mb-2 order-text">Wallet Balance</p>
                            </div>
                        </div>
                        <div class="col-md-6 col-12 mb-3">
                            <div class="bg-white shadow-sm rounded p-4 text-center" >
                                <p class="mb-3 text-primary order-count text-center">{{App\CentralLogics\Helpers::format_currency($cashInHand->balance)}}</p>
                                <p class="mb-2 order-text">Cash In Hand</p>
                            </div>
                        </div> 
                    </div>
                </div>
