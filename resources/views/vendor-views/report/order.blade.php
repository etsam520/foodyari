                            ...
                            <div class="dropdown me-3">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton22"
                                    ...
                                    <li><a class="dropdown-item"
                                            href="{!! route('vendor.report.order') . '?filter=this_week' !!}">This
                                            Week</a></li>
                                    <li><a class="dropdown-item"
                                            href="{!! route('vendor.report.order') . '?filter=this_month' !!}">This
                                            Month</a></li>
                                    <li><a class="dropdown-item"
                                            href="{!! route('vendor.report.order') . '?filter=this_year' !!}">This
                                            Year</a></li>
                                    <li><a class="dropdown-item"
                                            href="{!! route('vendor.report.order') . '?filter=previous_year' !!}">Previous
                                            Year</a></li>
                                    <li><a class="dropdown-item"
                                            href="{!! route('vendor.report.order') . '?filter=today' !!}">Today</a></li>
                                </ul>
                            </div>
                            <form action="">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title d-flex align-items-center">
                        <h5 class="page-header-title">Order Report</h5>
                        <span class="badge bg-primary ms-2 py-1" id="itemCount">{{ count($transactions) }}</span>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-responsive dataTable" id="datatable-export"
                            data-toggle="data-table-export" aria-describedby="datatable_info">
                            <thead>
                                <tr>
                                    <th>Order Id</th>
                                    <th>Date Time</th>
                            ...
                            @php
                                $sumFoodsAmount = $sumDiscountAmount = $sumCouponDiscountAmount = $sumofPackingCharge = $sumCommissionChargedByAdmin = $sumGrossTotal = $sumGstAmount = $sumReceivableAmount = $sumEarning = 0;
                            @endphp
                            <tbody>
                                ...
                                <tr>
                                    <td>{{$txn->order_id}}</td>
                                    <td>
                                        <small>{{Helpers::format_date($txn->created_at)}},
                                            {{Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $txn->created_at)->format('H:s:i A')}}
                                        </small> </h4>
                                    </td>

                                    <td>{{Helpers::format_currency($restaurantOrderData?->foodPriceCollectionPreDiscount)}}
                                    </td>
                                    @php($sumFoodsAmount += (float) $restaurantOrderData?->foodPriceCollectionPreDiscount)

                                    <td>{{Helpers::format_currency($restaurantOrderData?->sumOfDiscount)}}</td>
                                    @php($sumDiscountAmount += (float) ($restaurantOrderData?->sumOfDiscount))

                                    <td>{{$restaurantOrderData?->couponDiscount . " %"}}</td>

                                    <td>{{Helpers::format_currency($restaurantOrderData?->couponDiscountAmount)}}</td>
                                    @php($sumCouponDiscountAmount += (float) $restaurantOrderData?->couponDiscountAmount)

                                    <td>{{Helpers::format_currency($restaurantOrderData?->sumofPackingCharge)}}</td>
                                    @php($sumofPackingCharge += (float) $restaurantOrderData?->sumofPackingCharge)

                                    {{-- <td>{{Helpers::format_currency($restaurantOrderData?->commissionChargedByAdmin)}} --}}
                                    {{-- </td> --}}
                                    @php($sumCommissionChargedByAdmin += (float) $restaurantOrderData?->commissionChargedByAdmin)
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    {{-- <th><small> - Admin Commission
                                            <br>{{Helpers::format_currency($sumCommissionChargedByAdmin)}}</small></th> --}}
...
@push('javascript')
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#itemCount').text('{{ count($transactions) }}');
            $('#datatable-export').DataTable({
                dom: 'lBfrtip', // Add 'l' to include the "Show [x] entries" dropdown
                buttons: [
                ...
                lengthMenu: [[10, 25, 50, 100, 1000], [10, 25, 50, 100, 1000]], // Dropdown options for rows per page
