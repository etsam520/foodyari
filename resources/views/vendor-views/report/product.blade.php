                        ...
                        <div class="d-flex align-items-center flex-wrap">
                            ...
                            <div class="dropdown me-3">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuButton22" style="">
                                   <li><a class="dropdown-item" href="{!! route('vendor.report.product').'?filter=this_week' !!}">This Week</a></li>
                                   <li><a class="dropdown-item" href="{!! route('vendor.report.product').'?filter=this_month' !!}">This Month</a></li>
                                   <li><a class="dropdown-item" href="{!! route('vendor.report.product').'?filter=this_year' !!}">This Year</a></li>
                                   <li><a class="dropdown-item" href="{!! route('vendor.report.product').'?filter=previous_year' !!}">Previous Year</a></li>
                                   <li><a class="dropdown-item" href="{!! route('vendor.report.product').'?filter=today' !!}">Today</a></li>
                                </ul>
                            </div>
                            <form action="">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    <div class="header-title d-flex align-items-center">
                        <h5 class="page-header-title">Product Report</h5>
                        <span class="badge bg-primary ms-2 py-1" id="itemCount">{{ count($productItems) }}</span>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <table class="table table-responsive dataTable" id="datatable-export" data-toggle="data-table-export" aria-describedby="datatable_info">
                            ...
                            <tbody>
                                ...
                                {{-- @dd($product) --}}
                                    <tr>
                                        <td>
                                            <small>{{$key+1}}</small> </h4>
                                        </td>
                                        <td>{{Str::ucfirst($product['foodname'])}}</td>
                                        <td>{{ App\CentralLogics\Helpers::format_currency($product['optionPrice']) }}</td>
                                        {{-- <td>{{ $product['optionPrice'] }}</td> --}}

                                        <td>{{$product['quantity']}}</td>
                                        <td>{{App\CentralLogics\Helpers::format_currency((float) $product['total'])}}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
                                <tr>
                                    <th>##</th>
                                    <th> </th>
                                    <th><small>Sum Cost :</small> {{App\CentralLogics\Helpers::format_currency($sumcost)}}</th>
                                    <th><small>Quantity Total :</small> {{$sumquantity}}</th>
                                    <th><small>Sum Total :</small> {{App\CentralLogics\Helpers::format_currency($sumtotal)}}</th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
