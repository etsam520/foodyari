        ...
        <div class="page-header">
            ...
            <div class="my-2">
                <div class="row g-2 align-items-center justify-content-end">
                    ...
                    @endif
                    <div class="col-auto">
                        <a href="{{ route('vendor.food.add-new') }}" class="btn max-sm-12 btn--primary w-100"><i class="tio-add-circle"></i> {{ translate('messages.add') }} {{ translate('messages.new') }} {{ translate('messages.food') }}</a>
                    </div>
                ...
                <table id="datatable" class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                    ...
                    <thead class="thead-light">
                        <tr>
                            ...
                            <th class="text-center">{{ translate('messages.position') }}</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="media align-items-center" href="{{ route('vendor.food.view', [$food['id']]) }}">
                                        <img class="avatar avatar-lg mr-3" src="{{ asset('storage/app/public/product') }}/{{ $food['image'] }}" onerror="this.src='{{ asset('/public/assets/admin/img/100x100/food-default-image.png') }}'" alt="{{ $food->name }} image">
                                ...
                                <td>
                                    <div class="d-flex">
                                        <div class="mx-auto">
                                            <label class="toggle-switch toggle-switch-sm mr-2" data-toggle="tooltip" data-placement="top" title="{{ translate('messages.Change_food_visibility_to_customers') }}" for="stocksCheckboxSts{{ $food->id }}">
                                                <input type="checkbox" onclick="location.href='{{ route('vendor.food.status', [$food['id'], $food->status ? 0 : 1]) }}'"class="toggle-switch-input" id="stocksCheckboxSts{{ $food->id }}" {{ $food->status ? 'checked' : '' }}>
                                                <span class="toggle-switch-label">
                                                    <span class="toggle-switch-indicator"></span>
                                                </span>
                                            </label>
                                ...
                                <td>
                                    <form method="post" action="{{ route('vendor.food.updatePosition', ['id' => $food->id]) }}">
                                        @csrf @method('POST')
                                        <select id="position" name="position" onchange="this.form.submit()">
                                            <option value="">Select</option>
                                            @for ($i = 1; $i <= count($foods); $i++)
                                                <option value="{{ $i }}" {{ $i == $food->position ? 'selected' : '' }}>{{ $i }}</option>
                                            @endfor
                                        </select>
                                    </form>
                                </td>
                                <td>
                                    <div class="btn--container justify-content-center">
                                        <a class="btn action-btn btn--primary btn-outline-primary" href="{{ route('vendor.food.edit', [$food['id']]) }}" title="{{ translate('messages.edit') }} {{ translate('messages.food') }}"><i class="tio-edit"></i>
                                        </a>
                                        <a class="btn action-btn btn--danger btn-outline-danger" href="javascript:" onclick="form_alert('food-{{ $food['id'] }}','{{ translate('Want to delete this item ?') }}')"
                                            title="{{ translate('messages.delete') }} {{ translate('messages.food') }}"><i class="tio-delete-outlined"></i>
                                        </a>
