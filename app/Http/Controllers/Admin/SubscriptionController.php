...
use App\Models\AdminToMessSubscription;
use App\Models\AdminToRestaurantSubscriptonPackage;
use App\Models\Restaurant;
use App\Models\RestaurantSubscription;
use App\Models\RestaurantSubscriptionPakage;
use App\Models\RestaurantSubscriptionTransactions;
use App\Models\Zone;
use Carbon\Carbon;
...
class SubscriptionController extends Controller
{
    ...
    // }
    public function package_list($for)
    {
       if($for == 'restaurant'){
           $packages = AdminToRestaurantSubscriptonPackage::withCount('transactions')
               ->latest()
               ->paginate(15);
       }elseif($for == 'mess'){
            $packages = AdminToMessSubscription::withCount('transactions')
               ->latest()
               ->paginate(15);
       }
            // $expire_soon = RestaurantSubscription::where('status', 1)->whereDate('expiry_date', '<=', Carbon::today()->addDays('10'))->count();
    ...
    {
        $package = AdminToRestaurantSubscriptonPackage::findOrFail($id);
    ...
    public function store(Request $request)
    {
        // dd($request->post());
        $request->validate([
            'package_name' => 'required|max:191|unique:restaurant_subscription_pakages',
            ...
            'price.required' => __('Must enter Price for the Package'),
            'package_name.required' => __('Name of the Package is required'),
            'validity.required' => __('Must enter a validity period for the Package in days'),
        ]);

        if($request->subscription_for == "restaurant")
        {
            AdminToRestaurantSubscriptonPackage::create([
        ...
        }elseif($request->subscription_for == "mess"){
            AdminToMessSubscription::create([
                'package_name' => $request->package_name,
                'price' => $request->package_price,
                'validity' => $request->package_validity,
                'max_order' => $request->max_order  ?? 'unlimited',
            ...
            return back()->with('error', 'Failed To add Subscription Plan');
        }

        Session::flash('success',__('Subscription Plan Added Successfully'));
        return redirect()->route('admin.subscription.list',['for' => $request->subscription_for]);
    }
    ...
    public function update(Request $request)
    {
        ...
        AdminToRestaurantSubscriptonPackage::findOrfail($request->id)->update([
            'package_name' => $request->package_name,
            'price' => $request->package_price,
            'validity' => $request->package_validity,
            'max_order' => $request->max_order ?? 'unlimited',
        ...
        Session::flash('success',__('Subscription Plan Updated Successfully'));
    ...
    public function details($id)
    {
        ...
        $package = RestaurantSubscriptionPakage::withCount('transactions')->findOrFail($id);
        ...
        $on_trail=RestaurantSubscriptionTransactions::where('package_id',$package->id)->where('payment_method','free_trial')->count();
        ...
        $this_month_total_sub= RestaurantSubscriptionTransactions::where('package_id',$package->id)->whereMonth('created_at', Carbon::now()->month)->count();
        ...
        $his_month_on_trail=RestaurantSubscriptionTransactions::where('package_id',$package->id)->where('payment_method','free_trial')->whereMonth('created_at', Carbon::now()->month)->count();
        ...
        $this_year_total_sub= RestaurantSubscriptionTransactions::where('package_id',$package->id)->whereYear('created_at', Carbon::now()->year)->count();
        $this_year_total_on_package= RestaurantSubscription::where('package_id',$package->id)->whereYear('created_at', Carbon::now()->year)->count();
        $his_year_on_trail=RestaurantSubscriptionTransactions::where('package_id',$package->id)->where('payment_method','free_trial')->whereMonth('created_at', Carbon::now()->year)->count();

        $this_month = RestaurantSubscriptionTransactions::where('package_id', $id)->whereMonth('created_at', Carbon::now()->month)->sum('paid_amount');
        $transcation_sum = RestaurantSubscriptionTransactions::where('package_id', $id)->sum('paid_amount');

        $transcation_sum_month = RestaurantSubscriptionTransactions::where('package_id', $id)->whereMonth('created_at', Carbon::now()->month)->sum('paid_amount');
        $transcation_sum_year = RestaurantSubscriptionTransactions::where('package_id', $id)->whereYear('created_at', Carbon::now()->year)->sum('paid_amount');
        $transcation_sum = RestaurantSubscriptionTransactions::where('package_id', $id)->sum('paid_amount');
    ...
    public function transcation_list(Request $request, $id){
        ...
        $transcations = RestaurantSubscriptionTransactions::where('package_id',$id)
    ...
    public function transcation_search(Request $request)
    {
        ...
        $transcations = RestaurantSubscriptionTransactions::where(function ($q) use ($key) {
    ...
    {
        $key = explode(' ', $request['search']);
        $packages = RestaurantSubscriptionPakage::where(function ($q) use ($key) {
            foreach ($key as $value) {
    ...
    public function status(RestaurantSubscriptionPakage $package, Request $request)
    {
        $package->status = $request->status;
        $package->save();
        Session::flash('success',__('messages.Package') . __('messages.status_updated'));
        return back();
    }
    ...
    public function subscription_list(Request $request)
    {
        ...
        $transcations=RestaurantSubscriptionTransactions::get();
        $sub_transcations = $transcations->count();
        $total_earning = $transcations->sum('paid_amount');
        $this_month = RestaurantSubscriptionTransactions::whereMonth('created_at', Carbon::now()->month)->sum('paid_amount');
    ...
    //     $package = RestaurantSubscriptionPakage::findOrFail($request->package_id);
    ...
    //             Toastr::error(__('messages.You_can_not_downgraded_to_this_package_please_choose_a_package_with_higher_upload_limits') );
    //             return back();
    //             }
    ...
    //     }
    //     Toastr::success(__('messages.subscription_successful') );
    //     return back();
    // }
    ...
    //     Toastr::success(__('Subscription Canceled') );
    //     return back();
    // }
    ...
    //         'value' => $values,
    //     ]);
    //     Toastr::success(__('messages.free_trial_period_updated') );
    //     return back();
    // }
